...
TernSecure provides an additional option called `browserCookie` which uses cookies to persist the authentication state. This is particularly useful for server-side rendering (SSR) scenarios in Next.js, as cookies can be accessed on both the client and server sides.

browserSession persistence uses firebaseServerApp to render SSR. make sure to create
